...
- Native App Phase 2: Complete (branch `feat/mobile-phase2-screens-rsvp`, PR pending). Data hooks, tab screens (Home/Events/Clubs, Members stub), Event/Club detail, RSVP, TopBar, FAB, MenuDrawer.
- Native App Phase 3: Complete (branch `feat/mobile-phase3-parity`, awaiting on-device test). PWA feature parity minus the game layer, built via multi-agent workflows: foundation (theme tokens, 12 RN-Modal-based UI primitives, route skeleton, `packages/core/rpc.ts`), then 10 parallel work packages — Events tab (filters/calendar/past), Create Event wizard (templates/recurrence), Clubs + Create Club, Notifications inbox + preferences, Profile edit + delete account, Onboarding wizard, Event Detail (edit/cancel/recurring/attendees/share), Club Overview (stats/settings/guests/manage), Members + invites + join requests, Home dashboard + Discover + realtime (4 channels) — then integration (i18n harvest en/de/es, queryKeys/icons registries, invite-token persistence, dead-code sweep). Verified by driving the Expo-web build in Playwright; 3 web bugs found+fixed (share clipboard crash; delete-club + event-filter nested-modal unreachability). Deferred: game screens, live score, team generator, analytics, Discover map, Mapbox autocomplete, push. Device checklist: `apps/mobile/DEVICE_TEST_CHECKLIST.md`.
