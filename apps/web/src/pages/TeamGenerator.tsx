...
const TeamGenerator = () => {
  ...
  const balanceGenderDistribution = (
    teamA: PlayerWithPositions[],
    teamB: PlayerWithPositions[]
  ) => {
    ...
    if (
      Math.abs(malesTeamA - femalesTeamA - (malesTeamB - femalesTeamB)) <= 1
    ) {
      return { teamA, teamB };
    }
    ...
    for (let i = 0; i < teamA.length; i++) {
      for (let j = 0; j < teamB.length; j++) {
        ...
        if (
          playerA.preferredPosition === playerB.preferredPosition &&
          ...
          const newFemalesA =
            femalesTeamA + (playerB.gender === "female" ? 1 : -1);
          const newMalesB = malesTeamB + (playerA.gender === "male" ? 1 : -1);
          const newFemalesB =
            femalesTeamB + (playerA.gender === "female" ? 1 : -1);

          const currentDiff = Math.abs(
            malesTeamA - femalesTeamA - (malesTeamB - femalesTeamB)
          );
          const newDiff = Math.abs(
            newMalesA - newFemalesA - (newMalesB - newFemalesB)
          );
  ...
  const handleSaveMatch = (matchDetails: {
    date: string;
    location: string;
    notes: string;
  }) => {
                ...
                <GeneratedTeams
                  ...
                  onUpdateTeams={(newTeams) => {
                    ...
                    const convertedTeams = {
                      teamA: newTeams.teamA.map(
                        (p) =>
                          allPlayers.find((ap) => ap.name === p.name) ||
                          allPlayers[0]
                      ),
                      teamB: newTeams.teamB.map(
                        (p) =>
                          allPlayers.find((ap) => ap.name === p.name) ||
                          allPlayers[0]
                      ),
