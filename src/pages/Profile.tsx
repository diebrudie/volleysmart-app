...
const Profile = () => {
  ...
  const getPrimaryPosition = () => {
    ...
    return primary ? positions.find(p => p.id === primary.position_id)?.name : '';
  };
  ...
  const toggleSecondaryPosition = (positionId: string, checked: boolean) => {
    if (checked) {
      setPlayerPositions([...playerPositions, { position_id: positionId, is_primary: false }]);
    } else {
      setPlayerPositions(playerPositions.filter(pos => pos.position_id !== positionId));
    }
  };
          ...
          <TabsContent value="personal">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  {isOwnProfile ? "Update your personal details" : "Personal details"}
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="flex items-center space-x-6">
                  <Avatar className="h-24 w-24">
                    <AvatarImage 
                      src={imageFile ? URL.createObjectURL(imageFile) : profile.image_url || ''} 
                      alt="Profile" 
                    />
          ...
          <TabsContent value="skills">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  {isOwnProfile ? "Update your volleyball skills and preferences" : "Volleyball skills"}
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                <div>
                  ...
                  <Select
                    value={getPrimaryPosition() ? positions.find(p => p.name === getPrimaryPosition())?.id : ''}
                ...
                <div>
                  ...
                  <div className="mt-2 space-y-2">
                    {positions.map((position) => {
                      const isPrimary = playerPositions.some(p => p.position_id === position.id && p.is_primary);
                      const isSecondary = playerPositions.some(p => p.position_id === position.id && !p.is_primary);
                      ...
                      return (
                        <div key={position.id} className="flex items-center space-x-2">
                          <input
                            ...
                            onChange={(e) => toggleSecondaryPosition(position.id, e.target.checked)}
                            className="rounded"
                          />
                          <Label htmlFor={`secondary-${position.id}`} className={isPrimary ? "text-gray-400" : ""}>
                ...
                <div>
                  ...
                  <RadioGroup
                    ...
                    onValueChange={(value) => setProfile({ ...profile, skill_rating: parseInt(value) })}
                    disabled={!isOwnProfile}
